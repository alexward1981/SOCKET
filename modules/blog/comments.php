...
if (!empty($_POST['submit'])) {
   ...
   if(trim($_POST['user_comment']) == '' ) {
      ...
      $redFlag = 0;
		...
		$commentInsert = "INSERT INTO module_blog_comments (commentDetail, articleID, inReplyTo, userID, timeStamp, modRequired) VALUES ('$filteredInput', '$insertArticleID', '$inReplyTo', '$insertUserID', now(), '$redFlag')";
	...
	if ($inReplyTo > 0) {
		// if the comment is a reply, notify the person who posted the original comment
		$findOriginalComment = mysql_query("SELECT userID FROM module_blog_comments where commentID = $inReplyTo") or die ('failed: ' . mysql_error());
		$focResult = mysql_fetch_array($findOriginalComment, MYSQL_BOTH);
		extract ($focResult, EXTR_PREFIX_ALL, "FOC");
		$findCommenter = mysql_query("SELECT * FROM core_users WHERE userID = $FOC_userID") or die ('failed2: '.mysql_error());
		$FCResult = mysql_fetch_array($findCommenter, MYSQL_BOTH);
		extract ($FCResult,  EXTR_PREFIX_ALL, "FCR");
		//create a boundary string. It must be unique
		//so we use the MD5 algorithm to generate a random hash
		$random_hash = md5(date('r', time()));
		//define the headers we want passed. Note that they are separated with \r\n
		$headers = "From: noreply@digitalfusionmag.com\r\n";
...
$getTitle = mysql_query("SELECT articleID, articleTitle, permaLink, articleCat, articleAuthor, datePosted FROM module_blog WHERE (articleID = '".$_GET['article']."' OR permaLink = '".$_GET['article']."')");
...
$commentForm = '
<form enctype="multipart/form-data" id="commentForm" action="/blog/comments/'.strtolower($_GET['cat']).'/'.$_GET['article'].'" method="post">
'.$commentFormLabel.'
	'.$message.'
	<input name="articleID" type="hidden" value="'.$p_articleID.'" />
	<input name="inReplyTo" type="hidden" value="'.$_GET['cid'].'" />
	<input name="articleTitle" type="hidden" value="'.$p_articleTitle.'" />
	<input name="commenterID" type="hidden" value="'.$_SESSION['userID'].'" />
	<input name="username" type="hidden" value="'.$_SESSION['usr_username'].'" />
	<input name="category" type="hidden" value="'.strtolower($dbcat_categoryName).'" />
	<input name="thePermalink" type="hidden" value="'.$p_permaLink.'" />
...
// Selects all the comments in the database which reply to the article
$userComments = mysql_query("SELECT commentID, articleID, userID, commentDetail, timeStamp, modRequired FROM module_blog_comments WHERE inReplyTo = 0 AND modRequired = 0 AND articleID =" . $p_articleID . " ORDER BY commentID");
...
echo '<a class="hidden" name="comment'.$c_commentID.'" id="'.$c_commentID.'">Comment #'.$comCount.'</a>';
if ($_SESSION['userID']) {
...
$repCount = $comCount;
$userReply = mysql_query("SELECT commentID, articleID, userID, commentDetail, timeStamp, modRequired FROM module_blog_comments WHERE inReplyTo = ".$c_commentID." AND modRequired = 0 AND articleID =" . $p_articleID . " ORDER BY commentID");
...
echo '<div class="commentProfile">';
echo '<a class="hidden" name="comment',$r_commentID,'" id="',$r_commentID,'">Comment #',$repCount,'</a>';
if ($_SESSION['userID']) {
