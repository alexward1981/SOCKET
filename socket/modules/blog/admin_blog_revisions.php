...
{
   $dberase = "DELETE FROM module_blog_revisions WHERE revisionID = '{$_GET['delete']}'";
...
function deleteContent(revisionID)
{
   ...
   {
      window.location.href = 'admin_blog_revisions.php?ID=<?php echo $_GET['ID'] ?>&delete=' + revisionID;
   }
}
</script>
...
if(isset($_GET['republish']))
{
	...
	$republish = mysql_query("SELECT * FROM module_blog_revisions WHERE revisionID =".$_GET['republish']);
...
function republishContent(revisionID)
{
   ...
   {
      window.location.href = 'admin_blog_revisions.php?ID=<?php echo $_GET['ID'] ?>&republish=' + revisionID;
   }
}
</script>
...
echo $message;
$dblookup = "SELECT * FROM module_blog_revisions WHERE articleID = ".$_GET['ID']." ORDER BY revisionID DESC";
...
echo '<a href="'.SITEROOT.'/modules/blog/article.php?article=true&amp;revision='. $result['revisionID'].'"><img src="' . SITEROOT . '/socket/assets/images/buttons/button_open.png"  title="View revision on live site (not visible to the public)" /></a></td>';
