...
{
   $dberase = "DELETE FROM module_blog WHERE articleID = '{$_GET['delete']}'";
   mysql_query($dberase) or die('<h3 class="error"> Deletion Failed! </h3>' . mysql_error());
   $revisionerase = "DELETE FROM module_blog_revisions WHERE articleID = '{$_GET['delete']}'";
...
function deleteContent(articleID)
{
   ...
   {
      window.location.href = 'admin_blog.php?delete=' + articleID;
   }
}
</script>
...
function tweetContent(permaLink)
{
   ...
   {
      window.location.href = 'admin_blog.php?tweet=' + permaLink;
   }
}
</script>
...
if ($_GET['tweet']) {
	$getArticle = mysql_query("SELECT * FROM module_blog WHERE permaLink ='".$_GET['tweet']."'");
	...
	$catlookup = "SELECT categoryID, categoryName FROM module_blog_categories WHERE categoryID =" . $tweet_articleCat;
...
// Takes the url of the article and converts it into a is.gd url
	$longURL = SITEROOT.'/blog/'.$dbcat_categoryName.'/'.$_GET['tweet'];
...
echo $message;
$dblookup = "SELECT * FROM module_blog ORDER BY articleID DESC";
...
echo '<a href="'.SITEROOT.'/modules/blog/article.php?article='. $result['permaLink'].'"><img src="' . SITEROOT . '/socket/assets/images/buttons/button_open.png"  title="View article on live site" /></a></td>';
...
// SHOW COMMENTS
echo '<td ';
$modlookup = "SELECT modRequired FROM module_blog_comments WHERE articleID = " . $result['articleID'];
...
$commentlookup = "SELECT commentID, modRequired FROM module_blog_comments WHERE articleID = " . $result['articleID'];
...
if ($numcomments != 0) { echo '<a href="admin_blog_comments.php?ID='. $result['articleID'].'">'; }
...
// EDIT ARTICLES
echo '<td class="buttonCol2">';
echo '<a href="admin_blog_edit.php?ID='. $result['articleID'].'"><img src="' . SITEROOT . '/socket/assets/images/buttons/button_edit.png"  title="Edit Article" /></a></td>';

// SHOW REVISIONS
// First check to see if there are any revisions
$revisioncheck = mysql_query("SELECT revisionID FROM module_blog_revisions WHERE articleID =".$result['articleID']);
...
echo '<td class="buttonCol">';	
echo '<a href="admin_blog_revisions.php?ID='. $result['articleID'].'"><img src="' . SITEROOT . '/socket/assets/images/buttons/button_view_menus.png"  title="View Revisions" /></a></td>';
}
// DELETE ARTICLES
