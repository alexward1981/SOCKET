...
require_once('../../templates/standard/socket_header.php'); 
	...
	$titlelookup = "SELECT articleID, articleTitle FROM module_blog WHERE articleID=" . $_GET['ID'];
...
{
   $dberase = "DELETE FROM module_blog_comments WHERE commentID =" . $_GET['delete'];
...
function deleteContent(commentID)
{
   ...
   {
      window.location.href = 'admin_blog_comments.php?ID=<?php echo $articleID ?>&delete=' + commentID;
   }
}
</script>
<?php
...
{
   $dbapprove = "UPDATE module_blog_comments SET modRequired = 0 WHERE commentID =" . $_GET['approve'];
...
function approveContent(commentID)
{
   ...
   {
      window.location.href = 'admin_blog_comments.php?ID=<?php echo $articleID ?>&approve=' + commentID;
   }
}
</script>
<?php
...
{
   $dbblacklist = "UPDATE module_blog_comments SET modRequired = 2 WHERE commentID =" . $_GET['decline'];
...
function declineContent(commentID)
{
   ...
   {
      window.location.href = 'admin_blog_comments.php?ID=<?php echo $articleID ?>&decline=' + commentID;
   }
}
</script>
<?php
...
$dblookup = "SELECT commentID, articleID, userID, commentDetail, timeStamp, modRequired FROM module_blog_comments WHERE modRequired =0 AND articleID=" . $_GET['ID'];
...
$bllookup = "SELECT commentID, articleID, userID, commentDetail, timeStamp, modRequired FROM module_blog_comments WHERE modRequired =1 AND articleID=" . $_GET['ID'];
...
$bllookup = "SELECT commentID, articleID, userID, commentDetail, timeStamp, modRequired FROM module_blog_comments WHERE modRequired =2 AND articleID=" . $_GET['ID'];
$bldata = mysql_query($bllookup) or die('Failed to return data: ' . mysql_error());
$blsearchrows = mysql_num_rows($bldata);
