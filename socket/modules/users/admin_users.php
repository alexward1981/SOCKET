...
<div class="failure"><strong>Incomplete</strong><p>This section is not release ready</p></div>
<?php
if ($_GET['message']) { echo $_GET['message']; }

// Displays site only Global Adninistrators
echo '<h2>GLOBAL SOCKET ADMINISTRATORS </h2>';
$dblookup = "SELECT * FROM core_users WHERE usr_access_lvl <= 1 ORDER BY usr_access_lvl";
...
echo '    <img src="'.SITEROOT.'/assets/scripts/timthumb/timthumb.php?src='.$result['usr_avatar'].'&amp;w=35&amp;h=35&amp;zc=c" alt="'.$result['usr_username'].'\'s profile picture"/></td>';
} else
{
...
} elseif ($_SESSION['userID'] == $result["userID"]) {
echo '    <td class="buttons" width="32" align="center">';
echo '    <a href="admin_users_edit.php?ID='. $result["userID"].'"><img src="' . SOCKETROOT . '/assets/images/buttons/button_edit.png" width="32" height="32" alt="Edit User" /></a></td>';
} else { 
		echo '<td width="28" class="deadcol" align="center">';
		echo '<img src="' . SITEROOT . '/socket/assets/images/buttons/off_button_edit.png" width="32" height="32" alt="Cannot Edit User" /></td>'; }
// Admin switch yet to add
	...
	echo '<a href="javascript:deleteContent(\''. $result["userID"].'\');"><img src="' . SOCKETROOT . '/assets/images/buttons/button_delete.png" width="32" height="32" alt="Delete User" /></a></td>';
		...
		echo '<td width="28" class="deadcol" align="center">';
		echo '<img src="' . SOCKETROOT . '/assets/images/buttons/off_button_delete.png" width="32" height="32" alt="Cannot Delete User" /></td>'; }
echo '  </tr>';
...
// Displays site only staff
echo '<h2>'.strtoupper($sc_sitename).' STAFF </h2>';
$dblookup = "SELECT * FROM core_users WHERE usr_access_lvl BETWEEN 2 AND 5 ORDER BY usr_access_lvl";
$data = mysql_query($dblookup, $conn) or die('<h3 style="color:red"> Retrieval Failed! </h3>' . mysql_error());
...
// Displays site only the most recent public users

$dblookup = "SELECT * FROM core_users WHERE usr_access_lvl >= 5 ORDER BY userID DESC LIMIT 5";
$data = mysql_query($dblookup, $conn) or die('<h3 style="color:red"> Retrieval Failed! </h3>' . mysql_error());
...
$searchrows = mysql_num_rows($data);
	...
	{echo '<br /><h2 class="inline"> Recent Public Users </h2><p class="float_right button"> <a href="admin_public_users.php">View All</a> </p>';	
	echo '<table class="stripeMe" width="100%">';
		while($result = @mysql_fetch_array($data))
